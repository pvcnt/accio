...
final class ValueValidator {
  ...
  private def checkStringConstraint(values: Set[String], constraint: ArgConstraint, path: Option[String], errors: mutable.ListBuffer[InvalidSpecMessage]) = {
    constraint.allowedValues.foreach { allowedValues =>
      if (values.intersect(allowedValues).size < values.size) {
        errors += InvalidSpecMessage(s"Value must be one of {${allowedValues.mkString(",")}}")
      }
    }
  }
}
