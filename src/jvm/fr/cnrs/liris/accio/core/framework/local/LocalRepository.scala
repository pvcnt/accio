 ...
 * @tparam T Type of objects managed by this repository.
 */
abstract class LocalRepository[T: Manifest](mapper: FinatraObjectMapper, locking: Boolean) {
  protected def write(obj: T, file: File): Unit = {
  ...
  protected def read(file: File): Option[T] = withLock(file) {
    if (file.exists) {
