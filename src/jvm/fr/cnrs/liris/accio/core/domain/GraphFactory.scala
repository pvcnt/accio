...
final class GraphFactory @Inject()(opRegistry: OpRegistry) {
   ...
   * @throws InvalidGraphException If the graph definition is invalid.
   */
  ...
  def create(graphDef: GraphDef): Graph = {
    ...
    if (duplicateNodeNames.nonEmpty) {
      throw new InvalidGraphException(s"Duplicate node name: ${duplicateNodeNames.toSeq.sorted.mkString(", ")}")
    }
    ...
    val wiredNodes: Set[Node] = freeNodes.values.map { node =>
      node.inputs.foreach { case (thisPort, in) =>
        ...
        thisOp.inputs.find(_.name == thisPort) match {
          case None => throw new InvalidGraphException(s"Unknown port: ${node.name}/$thisPort")
          case Some(thisArg) =>
            in match {
              case ReferenceInput(ref) =>
                ...
                freeNodes.get(ref.node) match {
                  case None => throw new InvalidGraphException(s"Unknown input predecessor: ${ref.node}")
                  case Some(otherNode) =>
                    val otherOp = opRegistry(otherNode.op)
                    otherOp.outputs.find(_.name == ref.port) match {
                      case None => throw new InvalidGraphException(s"Unknown input predecessor port: ${ref.node}/${ref.port}")
                      case Some(otherArg) =>
                        //TODO: allow compatible types (e.g. byte can be converted into int).
                        if (otherArg.kind != thisArg.kind) {
                          throw new InvalidGraphException(s"Data type mismatch: ${otherNode.name}/${ref.port} => ${node.name}/$thisPort")
                        }
                    }
                }
    ...
    if (graph.roots.isEmpty) {
      throw new InvalidGraphException("No root found")
    }
    val cycles = detectCycles(graph)
    if (cycles.nonEmpty) {
      throw new InvalidGraphException(s"Cycles found: ${cycles.map(_.mkString(" -> ")).mkString(", ")}")
    }
  ...
  def validate(graphDef: GraphDef): Option[String] = {
    try {
      ...
      case e: InvalidGraphException => Some(e.getMessage)
    }
  }

  private def createNode(nodeDef: NodeDef) = {
    if (Utils.NodeRegex.findFirstIn(nodeDef.name).isEmpty) {
      throw new InvalidGraphException(s"Invalid node name: ${nodeDef.name}")
    }
    opRegistry.get(nodeDef.op) match {
      case None => throw new InvalidGraphException(s"Unknown operator: ${nodeDef.op}")
      case Some(opDef) =>
        Node(
          name = nodeDef.name,
          op = nodeDef.op,
          inputs = getInputs(nodeDef, opDef),
