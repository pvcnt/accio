...
class DatasetEnv(level: Int) extends StrictLogging {
  ...
  private[dataset] def submit[T, U: ClassTag](dataset: DataFrame[T], keys: Seq[String], processor: (String, Iterator[T]) => U): Array[U] = {
    if (keys.isEmpty) {
      Array.empty
    } else if (keys.size == 1) {
...
private class ParallelCollectionDataFrame[T: ClassTag](data: Map[String, Seq[T]], env: DatasetEnv)
 ...
 */
private class SourceDataFrame[T: ClassTag](source: DataSource[T], env: DatasetEnv) extends DataFrame[T](env) {
