...
/**
 * Local workflow repository configuration.
 *
 * @param rootDir Root directory where to store files.
 */
case class LocalWorkflowRepositoryConfig(rootDir: Path)
 ...
 */
final class LocalWorkflowRepository @Inject()(mapper: FinatraObjectMapper)
  extends LocalRepository(mapper, locking = false) with WorkflowRepository with LazyLogging with Configurable[LocalWorkflowRepositoryConfig] {

  override def configClass: Class[LocalWorkflowRepositoryConfig] = classOf[LocalWorkflowRepositoryConfig]

  override def find(query: WorkflowQuery): WorkflowList = {
    var results = config.rootDir.toFile.listFiles.toSeq
  ...
  override def save(workflow: Workflow): Unit = {
    if (exists(workflow.id, workflow.version)) {
      ...
      write(workflow, getPath(workflow.id, workflow.version).toFile)
    }
  }
  ...
  override def get(id: WorkflowId, version: String): Option[Workflow] = {
    read[Workflow](getPath(id, version).toFile)
  }
  ...
  private def getPath(id: WorkflowId) = config.rootDir.resolve(id.value)

  private def getPath(id: WorkflowId, version: String) = config.rootDir.resolve(id.value).resolve(s"$version.json")
