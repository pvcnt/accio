 ...
 */
final class ParseRunHandler @Inject()(runParser: RunParser) extends AbstractHandler[ParseRunRequest, ParseRunResponse] {
  override def handle(req: ParseRunRequest): Future[ParseRunResponse] = {
    ...
    try {
      ...
      case e: InvalidSpecException => Future(ParseRunResponse(errors = e.errors, warnings = e.warnings))
    }
  }
}