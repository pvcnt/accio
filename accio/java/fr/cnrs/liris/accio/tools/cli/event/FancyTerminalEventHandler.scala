...
import fr.cnrs.liris.accio.tools.cli.terminal._
 ...
 */
class FancyTerminalEventHandler(outErr: OutErr, opts: TerminalEventHandlerOptions)
  extends TerminalEventHandler(outErr, opts) with Logging {
   ...
   */
  private def progress(prefix: String, rest: String): Unit = {
  ...
  private def errorOrFail(event: Event): Unit = {
    previousLineErasable = false
    if (opts.useColor) {
      terminal.textRed()
      terminal.textBold()
  ...
  private def warning(warning: Event): Unit = {
    previousLineErasable = false
    if (opts.useColor) {
  ...
  private def info(event: Event): Unit = {
    previousLineErasable = false
    if (opts.useColor) {
      terminal.textGreen()
    }
   ...
   * @param event Event
   */
  private def putOutput(event: Event): Unit = {
    previousLineErasable = false
  ...
  private def writeTimestampAndLocation(event: Event): Unit = {
    if (opts.showTimestamp) {
