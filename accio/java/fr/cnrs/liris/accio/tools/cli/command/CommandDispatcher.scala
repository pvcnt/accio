...
private[command] case class CommandDispatcherOpts(
 ...
 */
final class CommandDispatcher @Inject()(registry: CommandRegistry, injector: Injector)
  extends Logging {
  ...
  def exec(args: Seq[String], outErr: OutErr): ExitCode = {
    ...
    registry.get(cmdName) match {
      ...
      case Some(meta) =>
        ...
        val opts = parser.as[CommandDispatcherOpts]
        setupLogging(Level.toLevel(opts.logLevel))
        ...
        val handler = createEventHandler(outErr, parser.as[TerminalEventHandlerOpts])
  ...
  private def createFlagsParser(cmdDef: Cmd) = {
    val flagClasses = cmdDef.flags ++ Seq(classOf[CommandDispatcherOpts], classOf[TerminalEventHandlerOpts])
    FlagsParser(cmdDef.allowResidue, flagClasses: _*)
  }

  private def parseArgsAndConfigs(parser: FlagsParser, args: Seq[String]): Unit = {
    parser.parse(args, Priority.CommandLine, Some("command line options"))
  }

  private def setupLogging(level: Level): Unit = {
    LoggerFactory.getLogger("fr.cnrs.liris.accio").asInstanceOf[Logger].setLevel(level)
    logger.info(s"Set logging level: $level")
  }

  private def createEventHandler(outErr: OutErr, opts: TerminalEventHandlerOpts) = {
    val handler = if (opts.useColor || opts.useCurses) {
      new FancyTerminalEventHandler(outErr, opts)
    } else {
      new TerminalEventHandler(outErr, opts)
    }
