...
final class DirectoryOpRegistry(directory: Path, checkFrequency: Duration) extends OpRegistry with Logging {
  ...
  private def updateLibraries(): Unit = {
    val keys = Files.list(directory).iterator.asScala.map { file =>
