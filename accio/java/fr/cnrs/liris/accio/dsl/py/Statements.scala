...
class Statements(indent: Int) {
  ...
  //   NEWLINE.map( _ => Nil) |
  //   (simple_stmt |
  //   compound_stmt.map(Seq(_)) ~ NEWLINE).rep.map(_.flatten)
  // )
  ...
  val single_input: P[Seq[Ast.stmt]] = P(
    NEWLINE.map(_ => Nil) |
      (simple_stmt |
        compound_stmt.map(Seq(_))).rep.map(_.flatten)
  )
  ...
  val decorator: P[Ast.expr] = P("@" ~/ dotted_name ~ ("(" ~ arglist ~ ")").? ~~ Lexical.nonewlinewscomment.? ~~ NEWLINE).map {
    ...
    case (name, Some((args, (keywords, starargs, kwargs)))) =>
      ...
      Ast.expr.Call(x, args, keywords, kwargs)
  }
  ...
  // )
  val small_stmt: P[Ast.stmt] = P(
    print_stmt | expr_stmt | return_stmt
  )
  ...
  //    // val assign = P( testlist ~ ("=" ~ (testlist.map(tuplize)|("(" ~ testlist.map(tuplize) ~ ")"))).rep )
  //     P(
  ...
  val expr_stmt: P[Ast.stmt] = {
    ...
    val assign = P(NAME.map(Ast.expr.Name(_, Ast.expr_context.Load)) ~ ("=" ~ (testlist.map(tuplize) | funcCall)).rep)

    // val assign = P( testlist ~ ("=" ~ (testlist.map(tuplize)|("(" ~ testlist.map(tuplize) ~ ")"))).rep )
    P(
      //aug.map{case (a, b, c) => Ast.stmt.AugAssign(tuplize(a), b, c) } |
      assign.map {
        case (a, Nil) => Ast.stmt.Expr(a)
        case (a, b) => Ast.stmt.Assign(Seq(a) ++ b.init, b.last)
      }
    )
  }

  // val expr_stmt: P[Ast.stmt] = P( NAME.map(Ast.expr.Name(_, Ast.expr_context.Load)) ~ ("=" ~ ( test)).rep ).map{
  //   case (a, b) => Ast.stmt.Assign(Seq(a) ++ b.init, b.last)
  // }
  //   val expr_stmt : P[Ast.stmt] = P(NAME.rep(1, ",") ~ ",".? ~ ("=" ~ testlist).? ).map{
  //     case (a, Nil) => Ast.stmt.Expr(tuplize(a.map(Ast.expr.Name(_,Ast.expr_context.Store))))
  //     case (a, b) => Ast.stmt.AssignList(a.map(Ast.expr.Name(_,Ast.expr_context.Store)),b)


  // val expr_stmt: P[Ast.stmt] = {
  //   //val aug = P( testlist ~ augassign ~ testlist.map(tuplize) )
  //   //val assign = P( testlist ~ ("=" ~ (yield_expr | testlist.map(tuplize))).rep )
  //   val assign = P( testlist ~ ("=" ~  testset1.map(tuplize)).rep )
  //   P(
  //     //aug.map{case (a, b, c) => Ast.stmt.AugAssign(tuplize(a), b, c) } |
  //     assign.map{
  //       case (a, Nil) => Ast.stmt.Expr(tuplize(a))
  ...
  val compound_stmt: P[Ast.stmt] = P(decorated)
