...
import fr.cnrs.liris.accio.dsl.py.WsApi._
...
object Expressions {
  ...
  //val funcCall : P[Ast.expr] = P(  NAME.map(Ast.expr.Name(_, Ast.expr_context.Call)) | NAME.map(Ast.expr.Name(_, Ast.expr_context.Call)) ~ "(" arglist ~ ")" ).map{

  // val funcCall : P[Ast.expr] = P(  NAME | NAME ~ "(" arglist ~ ")" ).map{
  //   case (a,Nil) => Ast.expr.Name(_, Ast.expr_context.Load)
  //       //case class Call(func: expr, args: Seq[expr], keywords: Seq[keyword], kwargs: Option[expr]) extends expr
  //   case (a,b) => Ast.expr.Call(a,)
  // }

  val funcCall: P[Ast.expr] = P(atom ~ trailer.rep).map {
    case (lhs, trailers) => trailers.foldLeft(lhs)((l, t) => t(l))
  }

  // val call = P("(" ~ arglist ~ ")").map{ case (args, (keywords, kwargs)) => (lhs: Ast.expr) => Ast.expr.Call(lhs, args, keywords, kwargs)}
  val trailer: P[Ast.expr => Ast.expr] = {
    val call = P("(" ~ arglist ~ ")").map { case (args, (keywords, starargs, kwargs)) => (lhs: Ast.expr) => Ast.expr.Call(lhs, args, keywords, kwargs) }
  ...
  val test: P[Ast.expr] = P(funcCall | atom)

  //val test : P[Ast.expr] = P( funcCall STRING.map(Ast.expr.Str) | NUMBER )
  //val expr : P[Ast.expr] = P( funcCall | STRING.map(Ast.expr.Str) | NUMBER )
  val expr: P[Ast.expr] = P(funcCall | atom)
  ...
  //val generator = P( list_contents ).map(Ast.expr.GeneratorExp.tupled)
  ...
  val testset1: P[Seq[Ast.expr]] = P("(" ~ test.rep(1, sep = ",") ~ ")")
