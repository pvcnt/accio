...
import fr.cnrs.liris.lumos.domain.LabelSelector
...
final class LumosServiceController @Inject()(jobStore: JobStore, eventHandler: EventHandler)
  ...
  override val pushEvent = handle(PushEvent) { args: PushEvent.Args =>
    eventHandler.handle(ThriftAdapter.toDomain(args.req.event))
      .map(_.toException)
      .flatMap {
  ...
  override val getJob = handle(GetJob) { args: GetJob.Args =>
    jobStore.get(args.req.name).flatMap {
      ...
      case None => Future.exception(ServerError.NotFound("jobs", args.req.name))
    }
  }
  ...
  override val listJobs = handle(ListJobs) { args: ListJobs.Args =>
    ...
    if (maybeSelector.exists(_.isLeft)) {
      Future.exception(ServerError.InvalidArgument(Seq(ServerError.FieldViolation(s"Cannot parse: ${args.req.labels}", "labels"))))
    } else {
      val query = JobQuery(
