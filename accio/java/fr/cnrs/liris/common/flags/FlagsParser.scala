 ...
 * A parser for flags.
 *
 * With `FooFlags` and `BarFlags` being be flags specification case classes whose all constructor fields are annotated
 * with @Flag(...), a typical use case in a main method is:
 *
 * {{{
 * val parser = FlagsParser(allowResidue = true, classOf[FooFlags], classOf[BarFlags])
 ...
 * }}}
 *
 * Alternatively, rather than calling `parseAndExitUponError`, client code may call `parse`, and handle parser
 * exceptions usage messages themselves.
 *
 * @param flagsData    Flags data.
...
object FlagsParser {
   ...
   */
  def apply(classes: Class[_]*): FlagsParser = apply(allowResidue = false, classes: _*)

  /**
   * Return a new flags parser for given classes.
   *
   * @param allowResidue Whether residual arguments are allowed.
   * @param classes      Flags-declaring classes.
   */
   ...
   * @return True if it corresponds to a boolean field, false otherwise.
   */
