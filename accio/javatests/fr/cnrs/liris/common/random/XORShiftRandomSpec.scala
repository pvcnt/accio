...
/**
 * Unit tests for [[XORShiftRandom]].
 */
class XORShiftRandomSpec extends UnitSpec {
   ...
   */
  it should "generate valid random numbers" in {
    val f = fixture
  ...
  it should "support a zero seed" in {
    val random = new XORShiftRandom(0L)
    assert(random.nextInt() != 0)
  }

  it should "produce random bits throughout" in {
   ...
   */
  private def times(numIters: Int)(f: => Unit): Unit = {
    var i = 0
    while (i < numIters) {
      f
      i += 1
