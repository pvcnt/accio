...
import fr.cnrs.liris.accio.framework.api.thrift.{ArgConstraint, ArgDef}
...
class ValueValidatorSpec extends UnitSpec {
  ...
  it should "validate strings" in {
    ...
    assertValid(
      Values.encodeString("string"),
      ArgDef("foo", DataType(AtomicType.String), constraint = Some(ArgConstraint(allowedValues = Set("str", "string")))))
  }
  ...
  it should "validate collections of strings" in {
    ...
    assertValid(
      Values.encodeList(Seq("string", "str"), AtomicType.String),
      ArgDef("foo", DataType(AtomicType.List, Seq(AtomicType.String)), constraint = Some(ArgConstraint(allowedValues = Set("str", "string")))))
    ...
    assertValid(
      Values.encodeSet(Set("string", "str"), AtomicType.String),
      ArgDef("foo", DataType(AtomicType.Set, Seq(AtomicType.String)), constraint = Some(ArgConstraint(allowedValues = Set("str", "string")))))
  }
