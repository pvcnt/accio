...
import fr.cnrs.liris.accio.core.domain.{AtomicType, DataType, Values}
...
class ListExplorationSpec extends UnitSpec {
  ...
  it should "expand into a list of integers" in {
    ...
    explo.expand(DataType(AtomicType.Integer)) should contain theSameElementsAs Set(10, 11, 12).map(Values.encodeInteger)
  }
}